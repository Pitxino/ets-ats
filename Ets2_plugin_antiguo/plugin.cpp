...
{
  return
    (value > 250.0f) ? 250 : // Sync byte is 255 so avoid that
    ((value < 0.0f) ? 0 :
    (unsigned char)(value));
}
...
#define GETFLTOPT(X) (option_file.get_option_float(X, 1.0f))
    ...
    PUT_BYTE_FLOAT(fabs(telemetry.speed) * 3.6 * GETFLTOPT("factor_speed")); // Absolute value for when reversing, dado en m/s en el dash_plugin 150 km/h en 250ª
